...
export const trackingApi = {
  async getNurseDriverLocation(
    nurseId: string,
    token: string | null,
  ): Promise<{ success: boolean; lat?: number; lng?: number; heading?: number; speed?: number; driverName?: string; message?: string }> {
    try {
      const response = await fetch(`${API_BASE_URL}/tracking/nurse/${nurseId}/driver-location`, {
        headers: {
          'Content-Type': 'application/json',
      ...
      const data = await response.json().catch(() => ({}));
      if (response.ok) {
        const src = data.driver ?? data.location ?? data.data ?? data;
        return {
          success: true,
          lat: src.lat != null ? Number(src.lat) : src.latitude != null ? Number(src.latitude) : undefined,
          lng: src.lng != null ? Number(src.lng) : src.longitude != null ? Number(src.longitude) : undefined,
          heading: src.heading != null ? Number(src.heading) : undefined,
          speed: src.speed != null ? Number(src.speed) : undefined,
          driverName: src.name ?? src.driver_name ?? src.driverName ?? undefined,
        };
      }
